...
parser.add('--data.test_way', type=int, metavar='N', help="classes per episode in test. 0 means same as data.way")
parser.add('--data.train_episodes', type=int, metavar='N', help="train episodes per epoch")
parser.add('--data.test_episodes', type=int, metavar='NTEST', help="test episodes per epoch")
...
# # Model
parser.add('--model.model_name', type=str,  metavar='RRNet/RRNet_small', help="model name")
...
# # Train
parser.add('--train.epochs', type=int, metavar='N', help='# epochs to train')
parser.add('--train.optim_method', type=str, help='optimization method')
parser.add('--train.learning_rate', type=float, metavar='FLOAT', help='learning rate')
parser.add('--train.decay_every', type=int, metavar='N', help='# epochs after which to decay the lr')
...
parser.add('--train.scheduler_type', type=str, metavar='STR', help='step/plateau')
parser.add('--train.plateau_patience', type=int, metavar='N', help='epochs to wait before cutting the LR if no improvement is observed')


# # Logs
parser.add('--log.fields', type=str, metavar='field1,field2,...', help="fields to monitor during training")
parser.add('--log.exp_dir', type=str, metavar='DIR',
                    help="directory where experiments should be saved (experiment date is automatically prepended)")
...
parser.add('--base_learner.learn_lambda', type=distutils.util.strtobool, metavar='BOOL',
            help="Learn the regularization coeff")
...
parser.add('--base_learner.linsys', type=distutils.util.strtobool, metavar='BOOL', help="If True uses torch.gesv instead of inverting matrix")
parser.add('--base_learner.method', type=str, metavar='R2D2/LRD2', help="Use LRD2 or R2D2")
